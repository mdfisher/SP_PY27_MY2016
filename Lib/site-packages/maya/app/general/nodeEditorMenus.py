...
def createNodeEditorMarkingMenu(ned):
    ...
    if inLookDev:
        cmds.menuItem(label=maya.stringTable[ 'y_nodeEditorMenus.kRemoveMaterialSoloing'  ], radialPosition = 'SE', c = unSoloCB, version = '2016', enable = (not inContainer))
...
def _createShaderMenuItems(ned, node):
    ...
    if groupSize <= 1:
        ...
        inLookDev = (nEd(ned, q=True, editorMode=True) == "lookdev")
        if inLookDev:
            cmds.menuItem(label=maya.stringTable[ 'y_nodeEditorMenus.kSoloMaterial'  ], radialPosition = 'S', c = soloCB, version = '2016')
            cmds.menuItem(label=maya.stringTable[ 'y_nodeEditorMenus.kRemoveMaterialSoloing'  ], radialPosition = 'SE', c = unSoloCB, version = '2016')
...
def createNodeItemMarkingMenu(ned, node):
    ...
    if not editMode:
        ...
        if cmds.ls(node, selection=True) or ( nedSelectionList and node in nedSelectionList ):
            parent = cmds.setParent(q=True, menu=True)
