...
class FileObjectInterruptedTestCase(unittest.TestCase):
    ...
    class MockSocket(object):
        def __init__(self, recv_funcs=(), timeout=None):
...
class NetworkConnectionNoServer(unittest.TestCase):
    class MockSocketTimeout(socket.socket):
        def connect(self, *args):
            raise socket.timeout('timed out')

    class MockSocketInterrupted(socket.socket):
        _eintr_triggered = False
        def connect(self, *args):
            if not self._eintr_triggered:
                self._eintr_triggered = True
                raise socket.error(errno.EINTR, "interrupted")

    @contextlib.contextmanager
    def mocked_socket_module(self, mock_class):
        """Return a socket which times out on connect"""
        old_socket = socket.socket
        socket.socket = mock_class
        try:
            yield
        finally:
    ...
    def test_create_connection_timeout(self):
        ...
        with self.mocked_socket_module(self.MockSocketTimeout):
            with self.assertRaises(socket.timeout):
                socket.create_connection((HOST, 1234))

    def test_create_connection_interrupted(self):
        with self.mocked_socket_module(self.MockSocketInterrupted):
            # This should not raise on an interrupted system call.
            socket.create_connection((HOST, 1234))
