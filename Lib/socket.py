    ...
    try:
        while True:
            if attempted and timeout:
                now = time.time()
                if now >= deadline:
                    raise error(errno.EWOULDBLOCK, "timed out")
                else:
...
class _fileobject(object):
        ...
        if self._wbuf:
            ...
            try:
                while write_offset < data_size:
                    ...
                    # The sendall method on a socket implements proper EINTR support.
    ...
    def read(self, size=-1):
        ...
        if size < 0:
            ...
            while True:
                data = _retryable_call(self._sock, self._sock.recv, rbufsize)
                if not data:
                    break
                buf.write(data)
            return buf.getvalue()
            ...
            while True:
                ...
                data = _retryable_call(self._sock, self._sock.recv, left)
                if not data:
                    break
                n = len(data)
    ...
    def readline(self, size=-1):
        ...
        if size < 0:
            ...
            self._rbuf = StringIO()  # reset _rbuf.  we consume it via buf.
            while True:
                data = _retryable_call(self._sock, self._sock.recv, self._rbufsize)
                if not data:
                    break
                nl = data.find('\n')
                if nl >= 0:
    ...
    *timeout* parameter will set the timeout on the socket instance
    ...
    for res in getaddrinfo(host, port, 0, SOCK_STREAM):
        ...
        try:
            ...
            _retryable_call(sock, sock.connect, sa)
            return sock
